...
func (config *config) setupWebsocketFlags() {
	...
	viper.AutomaticEnv()
	_ = viper.BindEnv("PublicIngestBaseUrls", "PUBLIC_INGEST_BASE_URLS")
	_ = viper.BindEnv("PrivateIngestBaseUrls", "PRIVATE_INGEST_BASE_URLS")
	...
	// If ingest base URLs are specified in the config file or env, use them
	if viper.IsSet("PublicIngestBaseUrls") {
		config.PublicIngestBaseUrls = viper.GetString("PublicIngestBaseUrls")
	}
	if viper.IsSet("PrivateIngestBaseUrls") {
		config.PrivateIngestBaseUrls = viper.GetString("PrivateIngestBaseUrls")
	}

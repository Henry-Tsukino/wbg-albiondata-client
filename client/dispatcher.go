...
func sendMsgToPublicUploaders(upload interface{}, topic string, state *albionState, identifier string) {
	...
	log.Debugf("Ingest URLs for topic '%v': %v", topic, PublicIngestBaseUrls)
...
func sendMsgToRedZoneUploader(upload interface{}, topic string, state *albionState, identifier string) {
	if ConfigGlobal.DisableUpload {
		log.Info("Upload is disabled.")
		return
	}
	...
	data, err := json.Marshal(upload)
	if err != nil {
		log.Errorf("Error while marshalling payload for RedZone event: %v", err)
		return
	}

	// Create HTTP uploader for the hardcoded Discord bot URL
	redZoneUploader := newHTTPUploader("https://discord-bot-production-adc0.up.railway.app")
	redZoneUploader.sendToIngest(data, topic, state, identifier)
}
